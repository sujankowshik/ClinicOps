...
const voiceTriageFlow = ai.defineFlow(
  ...
  async ({ audioDataUri, age }) => {
    ...
    const { text: symptoms } = await ai.generate({
      model: 'googleai/gemini-2.5-flash-speech-preview',
    ...
    // 2. Evaluate the transcribed symptoms.
    const evaluation = await evaluateSymptoms({ symptoms, age });

    // 3. Create a concise spoken response based on the evaluation.
    const responseText = `Based on the symptoms described, the potential diagnosis is ${evaluation.potentialDiagnoses[0]} with a severity of ${evaluation.severityAssessment}. It is recommended to see the ${evaluation.recommendedDepartment} department. ${evaluation.additionalRecommendations || ''}`;
